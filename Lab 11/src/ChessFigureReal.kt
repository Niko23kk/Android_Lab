class ChessFigureReal constructor(
    _id: Int,
    _name:String,
    _color:FigureColor
):ChessFigure{
    inner class FigureState {
        ...
        fun canThisFigureMove(isItMove: Boolean) {
            if (isItMove) {
                ...
                availableFigures[availableFigures.indexOfFirst { x -> x.id == id }] = this@ChessFigureReal
                return
            }
        ...
        fun canAttackOrBeingAttacked(isItMove: Boolean) {
            val closeFigures = availableFigures.count {
                it.position!![0] == this@ChessFigureReal.position!![0]
                        && it.color != this@ChessFigureReal.color
                        && (it.position!![1].toString().toInt() + 1 == this@ChessFigureReal.position!![1].toString().toInt()
                        || it.position!![1].toString().toInt() -1 == this@ChessFigureReal.position!![1].toString().toInt())
            }
    ...
    companion object {
        val availableFigures: MutableList<ChessFigureReal> = mutableListOf()
    ...
    fun getFigureMovementAbility() {
        when (this.name) {
            ...
            queenName -> println("Ferzin combines both types of movement from tower and elephant")
    ...
    private fun verifyCorrectAmountOfFigures() {
        ...
        when (this.name) {
            ...
            queenName -> {
                if (figures != null && figures.count { x -> x.color == this.color } >= amountOfQueen / 2) {
                    println("---Unable to add ${this.color} ${this.name} on position ${this.position} because it has reached limit")
                    return
                }
            }
    ...
    private fun generalVerifyCorrectAmountOfFigures(){

    }
}

fun ChessFigureReal.moveToNewPosition(position: String) {
    val storyItem: String = "${this.color} ${this.name} has moved from ${this.position} to $position"

    this.position = position
    ChessFigureReal.availableFigures[ChessFigureReal.availableFigures.indexOfFirst { x -> x.id == this.id }] = this

    ChessFigureReal.movementHistory.add(storyItem)
}

fun ChessFigureReal.removeFigureFromBoard() {
    this.position = null
    ChessFigureReal.availableFigures.removeIf { x -> x.position == null }
}